...
Workstream. For each cropped image:
- Load the image and convert it to grayscale.
- Load the corresponding interpolated grid lines mask (from Step 3).
- Extract left/right X coordinates for each column, using the mask. These define valid column boundaries.
- Dilate the grid mask horizontally by GRID_BUFFER_PCT to eliminate blobs too close to the table lines.
- Create a binary image excluding original grid pixels.
